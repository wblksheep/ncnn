...
检测
nihuihui写过一篇文章(https://zhuanlan.zhihu.com/p/128974102),这篇文章不纯讲去后处理后导出onnx,不过感觉还挺有借鉴作用的.在文章中提到.第三步：0x3 去掉后处理导出onnx.假设在这步出了问题。其实就是去找你使用的代码中要做结果输出的部分在那里修改代码，就可以导出onnx了。
